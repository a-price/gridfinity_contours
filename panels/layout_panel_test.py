"""Tests for the layout GUI panel (M6).
...
seconds, so a panel that quietly packed on a parameter change would lock
...
def test_editing_a_parameter_does_not_pack(qapp):
    """The whole reason this panel has a button."""
    panel = LayoutPanel(_quick())
    packs = []
    widget = panel.CreateWidget(on_change=lambda: packs.append(1))
...
def test_parameter_edits_still_reach_the_parameters(qapp):
    ...
    assert panel.parameters.pocket_offset == pytest.approx(2.5)
...
def test_the_panel_shows_what_the_offset_buys(qapp):
    ...
    """
    panel = LayoutPanel(_quick(pocket_offset=1.0))
    widget = panel.CreateWidget(on_change=lambda: None)
...
def test_packing_contours_produces_a_layout(qapp):
    ...
    panel.Run({0: _rectangle(20.0, 10.0), 1: _rectangle(18.0, 12.0)})

    assert panel.layout is not None
    assert panel.layout.cells <= 2
    assert len(panel.parts) == 2
...
def test_a_failure_reports_why_rather_than_just_failing(qapp):
    ...
    """
    panel = LayoutPanel(_quick(max_grid=1))
    widget = panel.CreateWidget(on_change=lambda: None)

    panel.Run({0: _rectangle(200.0, 30.0)})
    panel.RefreshStatus()
...
def test_a_pack_that_stepped_up_says_so(qapp):
    ...
    """
    panel = LayoutPanel(_quick())
    panel.parts = BuildParts({0: _rectangle(20.0, 10.0), 1: _rectangle(18.0, 12.0)})  # only counted here
    panel.result = PackResult(
...
def test_a_pack_that_found_the_first_feasible_size_makes_no_such_claim(qapp):
    panel = LayoutPanel(_quick())
    panel.parts = BuildParts({0: _rectangle(20.0, 10.0)})
    panel.result = PackResult(Layout(grid=(1, 1), placements={}), [GridAttempt((1, 1), PACKED)])

    assert "not ruled out" not in panel.Summary()
...
def test_running_a_pack_touches_no_widgets(qapp):
    ...
    assert panel.layout is not None, "the pack itself should still have happened"


def test_busy_swaps_which_button_is_live(qapp):
    panel = LayoutPanel(_quick())
    widget = panel.CreateWidget(on_change=lambda: None, on_cancel=lambda: None)
...
def test_progress_is_forwarded_to_the_caller(qapp):
    panel = LayoutPanel(_quick(max_grid=2))
    seen = []

    panel.Run({0: _rectangle(60.0, 25.0)}, progress=seen.append)

    assert seen, "expected the panel to pass the packer's progress on"
    assert all(report.restarts == panel.parameters.restarts for report in seen)
...
def test_a_cancelled_pack_says_so_rather_than_claiming_failure(qapp):
    ...
    """
    panel = LayoutPanel(_quick(max_grid=3))

    panel.Run({0: _rectangle(30.0, 25.0)}, cancelled=lambda: True)

    assert panel.layout is None
    assert panel.result is not None and panel.result.cancelled
    assert "cancelled" in panel.Summary().lower()
    assert "no fit" not in panel.Summary()
