...
and everything downstream re-runs synchronously the moment it returns. Two of
these run a search that takes minutes on a worker thread, so "re-run
downstream as soon as it returns" is exactly wrong - the display would
fire while the search was still going. They take `progress` and
`cancelled` callbacks and are sequenced by a finished signal instead. See
the note in `layout_gui`.
