"""Tests for the floorplan panel.

The property that matters here is that there is always something honest to
draw. This search runs for minutes, so a panel that could only render a
...
def _panel(drawers=None, **overrides) -> FloorplanPanel:
    ...
    return FloorplanPanel(params, [Drawer(6, 6)] if drawers is None else drawers)
...
def test_running_with_nothing_loaded_says_so_rather_than_raising():
    ...
    assert "load some contours" in panel.Summary()


def test_running_with_no_drawers_says_so():
    panel = _panel(drawers=[])

    panel.Run(_contours())

    assert panel.plan is None
    assert "drawer" in panel.Summary()
...
def test_a_library_no_drawer_could_store_is_reported_not_raised():
    ...
    panel = _panel(drawers=[Drawer(1, 1)])

    panel.Run(_contours())

    assert panel.plan is None
    assert panel.error is not None
    assert "Floorplan:" in panel.Summary()
...
def test_nothing_renders_without_a_drawer():
    ...
    drawer to draw.
    """
    assert _panel(drawers=[]).Render() is None
...
def test_every_drawer_is_drawn_including_the_empty_ones():
    ...
    """
    one = _panel(drawers=[Drawer(6, 6)]).Render()
    two = _panel(drawers=[Drawer(6, 6), Drawer(2, 2)]).Render()
...
def test_a_grouping_in_progress_is_drawn_in_the_drawers():
    ...
    panel = _panel()
    panel.Run(_contours())
    bins = tuple(panel.plan.layouts.values()) if panel.plan else ()
    empty = _panel().Render()
    plan, panel.plan = panel.plan, None
    assert plan is not None and empty is not None

    panel.SetProgress(Progress(GROUPING, 5, bins))
...
def test_an_assignment_in_progress_renders_as_a_partial_floorplan():
    ...
    panel.SetProgress(Progress(ASSIGNING, 9, bins, partial))
...
def test_a_bin_with_no_slot_is_drawn_beside_the_drawers():
    ...
    bins = tuple(panel.plan.layouts.values()) if panel.plan else ()
    panel.plan = None
    panel.drawers = [Drawer(1, 1)]  # nothing here can hold those bins
    bare = _panel(drawers=[Drawer(1, 1)]).Render()
...
def test_first_fits_opening_bins_are_drawn_as_they_are_opened():
    ...
    panel = _panel()
    panel.Run(_contours())
    one_bin = tuple(panel.plan.layouts.values())[:1] if panel.plan else ()
    empty = _panel().Render()
    panel.plan = None
    assert one_bin and empty is not None

    panel.SetProgress(Progress(FILLING, 2, one_bin, expected=3))
    ...
    assert "/3 objects into 1 bin(s)" in panel.Summary(), "a fraction of the library, not an arrangement of it"


def test_a_report_with_no_answer_yet_still_draws_the_drawers():
    panel = _panel()

    panel.SetProgress(Progress(GROUPING, 3))
...
def test_the_summary_follows_progress_while_running():
    ...
    panel.SetProgress(Progress(GROUPING, 7))

    assert "building the first" in panel.Summary()
...
def test_a_stopped_search_does_not_read_as_a_failure():
    ...
    panel.Run(_contours(), cancelled=lambda: True)

    assert panel.plan is not None and panel.plan.cancelled
    summary = panel.Summary()
...
def test_pinning_a_bin_keeps_it_across_a_replan():
    ...
    assert panel.plan is not None
    assert panel.plan.layouts[0] is held
    assert panel.PinnedIds() == frozenset([0])
...
def test_a_pin_outlives_the_plan_it_was_made_against():
    ...
    panel.drawers.append(Drawer(2, 2))
    panel.Clear()

    assert panel.pinned == [held], "Clear drops results, and a pin is an input"
...
def test_pinning_survives_a_session(tmp_path):
    ...
    panel.Save(path, contours)

    reopened = _panel(drawers=[])
    reopened.Load(path)
...
def test_the_summary_says_how_much_was_left_alone():
    ...
    assert "1 bin(s) pinned" in panel.Summary()
...
def test_exporting_writes_the_floorplan_and_every_bin_on_it(tmp_path):
    ...
    report = panel.Export(str(tmp_path / "plan"))
...
def test_an_exported_bin_is_the_one_the_plan_chose(tmp_path):
    ...
    panel.Run(_contours(4))
    assert panel.plan is not None
    layout = panel.plan.layouts[0]

    panel.Export(str(tmp_path / "plan"))
...
def test_the_bin_files_sort_the_way_the_report_lists_them(tmp_path):
    ...
    panel = _panel(max_grid=1)
    panel.Run({index: _rectangle(24.0, 24.0) for index in range(11)})
    assert panel.plan is not None and len(panel.plan.layouts) == 11
...
def test_pinned_bins_are_exported_too(tmp_path):
    ...
    assert report.bins == len(panel.Bins())
...
def test_a_solid_that_cannot_be_cut_is_reported_not_raised(tmp_path):
    ...
    panel.parameters.pocket_offset = 20.0
...
def test_exporting_before_planning_is_refused(tmp_path):
    with pytest.raises(ValueError, match="nothing planned"):
        _panel().Export(str(tmp_path / "plan"))
...
def test_exporting_a_search_stopped_before_the_drawers_still_works(tmp_path):
    ...
    panel.plan = replace(grouped, assignment=None, cancelled=True)
...
def test_exporting_a_search_stopped_before_any_bins_is_refused(tmp_path):
    ...
    panel = _panel()
    panel.Run(_contours(), cancelled=lambda: True)
    assert panel.plan is not None and not panel.plan.layouts
...
def test_a_session_round_trips_through_the_stage(tmp_path):
    ...
    assert reopened.drawers == panel.drawers
...
def test_a_session_saved_from_a_stopped_search_reopens_whole(tmp_path):
    ...
    panel.Run(contours)
    assert panel.plan is not None
    panel.plan = replace(panel.plan, assignment=None, cancelled=True)
...
def test_saving_before_planning_is_refused(tmp_path):
    with pytest.raises(ValueError, match="nothing planned"):
        _panel().Save(str(tmp_path / "s.json"), _contours())
...
def test_a_rerun_without_a_session_reports_no_churn():
    ...
    assert panel.changes is None
    assert "reprint" not in panel.Summary()
...
def test_editing_a_parameter_does_not_start_a_search(qapp):
    ...
    panel = _panel()
    runs = []
    widget = panel.CreateWidget(on_change=lambda: runs.append(1))
...
def test_stop_is_offered_only_to_a_host_that_can_stop(qapp):
    ...
    without = panel.CreateWidget(on_change=lambda: None)
    assert not _button(without, "Stop").isVisible()

    with_cancel = panel.CreateWidget(on_change=lambda: None, on_cancel=lambda: None)
...
def test_the_export_button_names_every_format_it_writes(qapp):
    panel = _panel()
    panel.CreateWidget(on_change=lambda: None)
