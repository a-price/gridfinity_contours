...
def _BinBody(
    layout: Layout,
    parts: dict[int, Part],
    pocket_offset: float | None,
    height_units: int,
    pocket_depth: float | None,
) -> tuple[str, float, float]:
    """The bin definition and its cutouts - everything `GenerateScad` emits
    except the header - plus the pocket offset and depth actually used, since
    both default and both are worth reporting.

    Shared by `GenerateScad`, which emits this at top level with one bin
    called `bin`, and `GenerateBinModule`, which wraps it in a named module
    for a scene holding several bins. All the same validation runs either
    way: a bin that cannot be cut on its own cannot be cut inside a scene
    either.
    """
    ...
    return body, pocket_offset, depth
...
def GenerateScad(
    ...
    """
    body, resolved_offset, depth = _BinBody(layout, parts, pocket_offset, height_units, pocket_depth)
    n, m = layout.grid
    return (
        f"// Generated by gridfinity_contours. {len(layout.placements)} pockets in a {n}x{m} bin.\n"
        f"// Pocket offset {resolved_offset}mm, depth {depth}mm.\n"
...
def GenerateBinModule(
    ...
    """The same bin `GenerateScad` writes, as a named module with no header
    of its own.

    For a scene composing several bins in one `.scad` file, which wants
    exactly one shared `include`/`use` rather than one per bin, and a
    distinct name per bin so a caller can `translate()`/`rotate()` and
    instantiate each independently. `bin` itself needs no renaming to avoid
    colliding across modules - OpenSCAD gives every module its own variable
    scope, so two modules each defining `bin` never see each other's.
    """
    body, _, _ = _BinBody(layout, parts, pocket_offset, height_units, pocket_depth)
